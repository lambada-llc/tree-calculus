...
The honest finding: **flat scaling** on this suite (fib/silly/merge, t1≈t4), plus
~2× single-thread OpenMP overhead. Two causes: (1) the interpreter's duplication
rule is fine-grained and *unbalanced* — fib's two branches differ in size by φ,
and a fixpoint combinator serializes the traversal, so one task gets nearly all
the work; (2) these programs are fold/fixpoint-bound, so the top-level chain is
sequential. An early version even overflowed the 31-bit id space by spawning a
chunk-per-task.

What would actually scale: parallelize where the *data* is independent (a map
over a large list; balanced divide-and-conquer), with cost-estimated granularity
and a proper work-stealing deque — not at every interpreter-level branch. Realistic
target on a genuinely data-parallel program: ~3–4× on 4 cores, the only avenue
that can beat a perfect sequential evaluator by a multiple.

```sh
clang++ implementation/cpp/parallel-packed.cpp -O3 -std=c++23 -stdlib=libc++ \
  -fopenmp -o implementation/cpp/parallel-packed
OMP_NUM_THREADS=4 BCUT=6 implementation/cpp/parallel-packed < input
```
...
| Parallel fork-join | prototype (`parallel-packed.cpp`); flat here, design for data-parallel |
