  ...
  switch (term.variant) {
    ...
    case 'App':
      if (!contains(name, term.b))
        // return c2(elim_star_skibc_eta(name, term.a), term.b);
        return app(c, elim_star_skibc_op_eta(name, term.a), term.b);
      if (!contains(name, term.a))
        if (term.b.variant === 'Var' && term.b.name === name)
          return term.a;
        else
          // return b2(term.a, elim_star_skibc_eta(name, term.b));
          return app(b, term.a, elim_star_skibc_op_eta(name, term.b));
      // return s2(elim_star_skibc_eta(name, term.a), elim_star_skibc_eta(name, term.b));
