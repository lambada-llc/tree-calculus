...
// This version executes each round in parallel to turn that into wall-clock
// speedup.
//
...
//     banks (parallel atomic batch-append — one fetch_add + memcpy per thread
//     per round, no serial merge).
//
// Node = two 32-bit words, 8 bytes: the tag lives in the top 3 bits of the first
// word (indices use the low 29 bits, up to 2^29 ≈ 537M nodes). Reduction is
// bandwidth-bound — an application node per reduction — so packing 12 -> 8 bytes
// cuts traffic on both the single-thread and the scaling axis.
//
